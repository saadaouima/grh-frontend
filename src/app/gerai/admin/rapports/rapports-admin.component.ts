...
@Component({
  ...
  templateUrl: './rapports-admin.component.html',
  styleUrls: ['./rapports-admin.component.scss'],
...
export class RapportsAdminComponent implements OnInit {
  ...
  readonly departments = [
    ...
    'Commercial', 'Data', 'Infrastructure', 'Direction'
  ];
  ...
  private readonly columnsMap: Record<TabType, string[]> = {
    CONGES:      ['employe', 'departement', 'type', 'debut', 'fin', 'jours', 'statut'],
  ...
  onDeptChange(): void {
    this.rows = this.applyDeptFilter(this.allRows);
    this.cdr.markForCheck();
  }

  exportReport(): void {
    if (this.formFormat === 'PDF') this.exportPdf();
  ...
  private loadTab(tab: TabType): void {
    ...
    this.rapportService.getRapport(tab).subscribe({
      next: (data: RapportData) => {
        ...
        this.rows    = this.applyDeptFilter(data.rows);
        this.loading = false;
        this.cdr.markForCheck();
      },
      error: () => {
  ...
  private applyDeptFilter(rows: RapportRow[]): RapportRow[] {
    if (!this.formDept) return rows;
    return rows.filter(r =>
      String(r['departement'] ?? '').toLowerCase().includes(this.formDept.toLowerCase())
    );
  }

  // ── Computed ──────────────────────────────────────
  get currentColumns(): string[] { return this.columnsMap[this.activeTab]; }
  get currentTabLabel(): string  { return this.tabs.find(t => t.value === this.activeTab)?.label ?? ''; }
  ...
  exportPdf(): void {
    ...
    this.http.get(`/api/rapports/export/${tab}`, { responseType: 'blob' }).subscribe({
      next: blob => {
        ...
        link.href = url; link.download = `rapport-admin-${tab}.pdf`;
  ...
  exportExcel(): void {
    ...
    this.http.get(`/api/rapports/export-excel/${tab}`, { responseType: 'blob' }).subscribe({
      next: blob => {
        const url  = window.URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url; link.download = `rapport-admin-${tab}.xlsx`;
        link.click(); window.URL.revokeObjectURL(url);
        this.isExporting = false; this.cdr.markForCheck();
      },
      error: () => {
