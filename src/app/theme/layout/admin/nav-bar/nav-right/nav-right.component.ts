...
export class NavRightComponent implements OnInit {
  ...
  loadUserProfile(): void {
    try {
      ...
      this.profileLinks = isChef ? PROFILE_LINKS_CHEF : PROFILE_LINKS_EMPLOYE;

      this.userProfile = {
        nom: token?.['family_name'] || 'Utilisateur',
        prenom: token?.['given_name'] || '',
        email: token?.['email'] || '',
  ...
  updateUnreadCount(): void { this.unreadCount = this.notifications.filter(n => !n.read).length; }
  markAllAsRead(): void { this.notifications.forEach(n => n.read = true); this.updateUnreadCount(); }
  ...
  getFullName(): string { return `${this.userProfile.prenom} ${this.userProfile.nom}`.trim() || 'Utilisateur'; }
  getInitiales(): string {
